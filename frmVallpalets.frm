...
Dim b As Boolean
    ...
    If CadenaDesdeOtroForm <> "" Then
        ...
        Else
            ...
            'Hay que poner en los campos lo que toca
            'CadenaDesdeOtroForm
            Text1(1).Text = RecuperaValor(CadenaDesdeOtroForm, 1)
            ...
            'Refrescamos
            CadenaConsulta = "Select * from tmpinformes  WHERE codusu =" & vUsu.Codigo
            If RecuperaValor(CadenaDesdeOtroForm, 2) = "" Then
                CadenaConsulta = CadenaConsulta & " AND campo2=" & RecuperaValor(CadenaDesdeOtroForm, 3)
                Text1(0).Text = RecuperaValor(CadenaDesdeOtroForm, 3)
            Else
                CadenaConsulta = CadenaConsulta & " AND Codigo1=" & RecuperaValor(CadenaDesdeOtroForm, 2)
                Text1(0).Text = RecuperaValor(CadenaDesdeOtroForm, 2)
            End If
            CadenaConsulta = CadenaConsulta & " ORDER BY nombre2,codigo1"
...
'    End If
    ...
    Select Case Button.Index
        ...
        Case 10
            ...
            'tmptraza( codusu,artppal,numloteppal,contador,idOperacion,artic2,cantidad,abs_cantidad
            Cad = ""
            Aux = ", (" & vUsu.Codigo & "," & DBSet(Text1(0).Text & " " & Text2(0).Text, "T") & ","
            Aux = Aux & DBSet(Text1(1).Text & " " & Text2(1).Text, "T") & ","
            
            For NumRegElim = 1 To ListView1.ListItems.Count
                Cad = Cad & Aux & NumRegElim & ","
                Cad = Cad & "'" & ListView1.ListItems(NumRegElim).Text & "',"
                Cad = Cad & DBSet(ListView1.ListItems(NumRegElim).SubItems(1) & " " & ListView1.ListItems(NumRegElim).SubItems(2), "T") & ","
                Cad = Cad & Val(Trim(ListView1.ListItems(NumRegElim).SubItems(3))) & " , " & Val(Trim(ListView1.ListItems(NumRegElim).SubItems(4)))
                Cad = Cad & "," & Val(Trim(ListView1.ListItems(NumRegElim).SubItems(5))) & ")"
            Next
            
            Aux = "INSERT INTO tmptraza( codusu,artppal,numloteppal,contador,idOperacion,artic2,cantidad,abs_cantidad,nomclien) VALUES "
            Cad = Mid(Cad, 2)
            Aux = Aux & Cad
            conn.Execute Aux
            
            Cad = "{tmptraza.codusu} = " & vUsu.Codigo
            LlamaImprimirGral Cad, "", 0, "vallMoviPalots.rpt", "Movi, PALOTS"
        ...
        Case 11
            ...
            CadenaDesdeOtroForm = ListView1.SelectedItem.SubItems(2) & "|" & Format(ListView1.SelectedItem.Text, "dd/mm/yyyy") & "|"
            frmVarios.Opcion = 13
...
Private Sub CargarDatosEnTemporal()
    ...
    lblIndicador.Caption = "Leyendo datos palets"
        ...
        conn.Execute "DELETE FROM tmpinformes where codusu=" & vUsu.Codigo
        Espera 0.15
        CadenaConsulta = "INSERT INTO tmpinformes(codusu,codigo1,campo2,nombre2) VALUES  " & Mid(CadenaConsulta, 2)
        conn.Execute CadenaConsulta
        Set miRsAux = Nothing
