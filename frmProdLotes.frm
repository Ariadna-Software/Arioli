...
Begin VB.Form frmProdLotes 
   BorderStyle     =   3  'Fixed Dialog
   ClientHeight    =   5145
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4680
   Icon            =   "frmProdLotes.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5145
   ...
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmProdLotes.frx":000C
      Height          =   3270
      Left            =   120
      TabIndex        =   9
      TabStop         =   0   'False
   ...
   End
End
Attribute VB_Name = "frmProdLotes"
...
Dim anc As Single
    ...
    CadenaConsulta = "select sum(cantlote) total ,max(linea) ultimo from sliordprlotes " & DevWHERE
...
Dim cad As String
    ...
    cad = Mid(cad, 8)
    cad = cad & " AND 1 "
    cad = DevuelveDesdeBD(conAri, "sum(cantlote)", "sliordprlotes", cad, "1")
    If cad = "" Then cad = "0"
...
On Error GoTo Error2
    ...
    If MsgBox(SQL, vbQuestion + vbYesNo) = vbYes Then
        ...
        SQL = DevWHERE
        SQL = "Delete FROM sliordprlotes " & SQL & " AND linea = " & adodc1.Recordset!Linea
        Conn.Execute SQL
        CancelaADODC adodc1
        CargaGrid
...
Private Sub Form_Activate()
    ...
    If Me.lblDesc.Caption = "" Then
        ...
        Me.lblDesc.Caption = "PRO: " & Format(Me.vIdProd, "000000") & " - " & Me.vCodartic
...
Private Sub Form_Load()
    ...
    End With
    Me.Caption = "Lotes" & Space(20) & "PRODUCCION"
...
Private Function DevWHERE() As String
    ...
    DevWHERE = " where codigo=" & Me.vIdProd & " AND codalmac =" & Me.vCodAlmac & " AND codartic =" & DBSet(vCodartic, "T")
End Function
...
Dim SQL As String
    ...
    SQL = DevWHERE
    ''sliordprlotes` (`codigo`,`codalmac`,`codartic`,`linea`,`numlote`,`cantlote`)
    SQL = "Select linea,numlote,cantlote from sliordprlotes" & SQL
...
Dim Can As Currency
    ...
    'FALTA###
    If Modo = 3 Then
        SQL = "INSERT INTO `sliordprlotes` (`codigo`,`codalmac`,`codartic`,`linea`,`numlote`,`cantlote`) values ("
        SQL = SQL & Me.vIdProd & "," & Me.vCodAlmac & ",'" & Me.vCodartic & "',"
        SQL = SQL & Linea & ",'" & DevNombreSQL(Lote) & "'," & DBSet(Cantidad, "N") & ")"
    Else
        'Modificar

        SQL = "UPDATE sliordprlotes SET numlote = '" & DevNombreSQL(Lote) & "' "
