...
Begin VB.Form frmVallTransOliva 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Empresas transporte oliva"
   ClientHeight    =   6015
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4710
   Icon            =   "frmVallTransOliva.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6015
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   3
      TabIndex        =   0
      Tag             =   "Código |N|N|0||vallempresatransoliva|codEmpre|000|S|"
      Text            =   "Dat"
      Top             =   4920
      Width           =   800
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   25
      TabIndex        =   1
      Tag             =   "Nombre |T|N|||vallempresatransoliva|NomEmpre||N|"
      Text            =   "Dato2"
      Top             =   4920
      Width           =   1395
   End
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmVallTransOliva.frx":000C
      Height          =   4725
      Left            =   120
      TabIndex        =   8
      TabStop         =   0   'False
   ...
   End
End
Attribute VB_Name = "frmVallTransOliva"
...
Dim anc As Single
    ...
    txtAux(0).Text = SugerirCodigoSiguienteStr("vallempresatransoliva", "codempre")
...
Dim SQL As String
    ...
    If adodc1.Recordset.EOF Then Exit Sub
    If Val(adodc1.Recordset!codempre) = 0 Then Exit Sub 'La CERO no se elimina nunca
    
    
    SQL = DevuelveDesdeBD(conAri, "EmpresaTransporte", "vallentradacamion", "EmpresaTransporte", CStr(adodc1.Recordset!codempre))
    If Trim(SQL) <> "" Then
        MsgBox "No se puede eliminar la empresa. Esta vinculada a entrada de mercancia", vbExclamation
        Exit Sub
    End If
    
    
    '### a mano
    SQL = "¿Seguro que desea eliminar la empresa de transporte?" & vbCrLf
    ...
    If MsgBox(SQL, vbQuestion + vbYesNo) = vbYes Then
        ...
        SQL = "Delete from vallempresatransoliva where codempre=" & adodc1.Recordset!codempre
        conn.Execute SQL
...
Dim cad As String
    ...
    cad = Abs(DatosModificados) & "|"
    cad = cad & adodc1.Recordset.Fields(0) & "|"
...
Private Sub Form_Load()
    ...
    'nomempre codempre
    'Cadena consulta
    CadenaConsulta = "Select codempre ,nomempre from vallempresatransoliva "
    CargaGrid
End Sub
